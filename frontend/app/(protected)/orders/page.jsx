...
export default function OrdersPage() {
  ...
  return (
    <>
      <div data-id="orders-page" className="pb-28">

        {/* Skeletons */}
        {loading && (
          <div data-id="orders-skeleton" className="mx-4 mt-4 rounded-2xl border border-border overflow-hidden bg-card">
            {[...Array(4)].map((_, i) => (
              <div key={i}>
        ...
        {!loading && comandas.length === 0 && (
          <div data-id="orders-empty-state" className="flex flex-col items-center justify-center py-24 px-8 text-center">
        ...
        {/* Lista */}
        {!loading && comandas.length > 0 && (
          <div data-id="orders-header" className="px-4 pt-4 flex flex-col gap-1.5">
            <p data-id="orders-count-label" className="text-xs text-muted-foreground px-1 mb-1">
              {comandas.length} aberta{comandas.length !== 1 ? "s" : ""}
            </p>
            <div data-id="orders-list" className="rounded-2xl border border-border bg-card overflow-hidden">
              {comandas.map((c, i) => (
